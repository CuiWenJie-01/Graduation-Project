...
import com.express.domain.bean.OrderPayment;
import com.express.domain.enums.*;
...
import com.express.mapper.OrderInfoMapper;
...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
    ...
    public ResponseResult createOrder(OrderInfo orderInfo, double money, String uid) {
       ...
       String orderId = orderInfo.getId();
        boolean b = orderPaymentService.createAliPayment(orderId, money);
        if(!b) {
            transactionManager.rollback(status);
    ...
    public Map<String, Long> getAdminDashboardData() {
        ...
        Long waitCount = orderInfoMapper.selectCount(new QueryWrapper<OrderInfo>().eq("status", OrderStatusEnum.WAIT_DIST.getStatus()));
        Long transportCount = orderInfoMapper.selectCount(new QueryWrapper<OrderInfo>().eq("status", OrderStatusEnum.TRANSPORT.getStatus()));

        map.put("today", Long.parseLong(list.size()+""));
    ...
    public Map<String, Long> getUserDashboardData(String userId) {
        ...
        long waitCount = 0, transportCount = 0, waitPaymentCount = 0;
    ...
    @Override
    public Map<String, Long> getCourierDashboardData(String courierId) {
        Map<String, Long> map = new HashMap<>();

        Long waitCount = orderInfoMapper.selectCount(new QueryWrapper<OrderInfo>()
                .eq("status", OrderStatusEnum.WAIT_DIST.getStatus()));

        Long transportCount = orderInfoMapper.selectCount(new QueryWrapper<OrderInfo>()
