...
import com.express.common.util.DateUtils;
...
import com.express.domain.enums.ThirdLoginTypeEnum;
import com.express.domain.vo.BootstrapTableVO;
import com.express.domain.vo.admin.AdminUserInfoVO;
import com.express.domain.vo.user.UserInfoVO;
import com.express.mapper.SysUserMapper;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Map<String, Long> getAdminDashboardData() {
        ...
        Long todayCount = sysUserMapper.selectCount(new QueryWrapper<SysUser>().between("create_date",
                LocalDateTime.of(LocalDate.now(), LocalTime.MIDNIGHT), LocalDateTime.now()));

        Long totalCount = sysUserMapper.selectCount(null);

        Long disEnableCount = sysUserMapper.selectCount(new QueryWrapper<SysUser>().eq("has_enable", 0));

        Long lockCount = sysUserMapper.selectCount(new QueryWrapper<SysUser>().gt("lock_date", LocalDateTime.now()));
