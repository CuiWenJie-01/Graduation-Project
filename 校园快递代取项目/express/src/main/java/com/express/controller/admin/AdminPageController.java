package com.express.controller.admin;
...
public class AdminPageController {
    ...
    public String showDashboardPage(@AuthenticationPrincipal SysUser sysUser, ModelMap map) {
        ...
        Map<String, Long> data = sysUserService.getAdminDashboardData();

        String userDesc = "今日注册用户数：" + data.get("today") +
                "，总用户数：" + data.get("total") +
                "，其中禁用用户数：" + data.get("disEnable") +
                "，冻结用户数：" + data.get("lock");
        map.put("userDesc", userDesc);

        Map<String, Long> data1 = orderInfoService.getAdminDashboardData();
        String orderDesc = "今日新增订单数：" + data1.get("today") +
                "，总等待接单数：" + data1.get("wait") +
                "，正在派送数：" + data1.get("transport");
        map.put("orderDesc", orderDesc);

        Map<String, Long> data2 = feedbackService.getAdminDashboardData();
